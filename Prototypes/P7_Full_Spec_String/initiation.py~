...
def p5_run():
    ...
    save(saves)
    if "second_string" not in saves or debug:
        print("Creating a second iteration string")
        saves["second_string"] = saves["first_string"].prep_new_string()
    save(saves)
    print("first end:\n", saves["first_string"].end_loc)
    print("targets from first:\n", saves["first_string"].get_CVs())
    print("second begining:\n", saves["second_string"].get_CVs())
    #print("
    save(saves)

...
class VIS:
    ...
    def __init__(self,
                 ...
                 path = None):
        if path == None:
            raise DebugException("VIS generated without path")
        self.main_path = path
        try:
            os.mkdir(path)
            ...
            pass
        #    print(str(e)+ 5*"\n")            
        #    raise DebugException("VIS with duplicate directory")
    ...
    def delete_runs(self):
        ...
        raise DeprecatedError("delete_runs is no longer to be used")
    """
        if True:
            return
    ...
    def fresh_copy(original, collection = None, new_name = None):
        if new_name == None:
            raise DebugException("copy name not designated")
        copy_no = 0
        name_found = False
        base_name = "/".join(original.main_path.split("/")[:-1])
        if new_name == "":
            while not name_found:
                try:
                    new_name = base_name + "_copy_" + str(copy_no) + "/"
                    os.listdir(new_name)
                    copy_no += 1
                except FileNotFoundError:
                    name_found =  True
        log("Creating VIS copy named: " + new_name)
    ...
    def nvt(self, new_parameters = None, new_keys = None):
        ...
        rem_path = self.single_run(prep, sim_name)
        print("nvt")
        shutil.move(rem_path + "state.cpt", self.latest_path + "state.cpt")
        if rem_path != None:
            shutil.rmtree(rem_path)
    ...
    def steered(self, new_parameters = None, new_keys = None):
        ...
        prep = self.single_prep(name = "steered",
                                template = template,
                                additions = additions)
        old_path = self.single_run(prep, sim_name)
        return old_path

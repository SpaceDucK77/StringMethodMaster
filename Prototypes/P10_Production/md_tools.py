...
def create_panda(start,
    ...
    dict_base = {}
    names = ["Iter", "Bead_no"] + list(CV_names)
...
def create_string(start, end, intermediaries, delta, opposites, saves, collection, run_time = 500, name = None):
    # Creates an initial string for the string method
    starter = VIS.VIS.fresh_copy(start, new_name = name)
    n = len(intermediaries)
    if "path" in saves:
        cv_span = saves["cv_span"]
        path = saves["path"]
    else:
        cv_span={"nsteps": str(int(run_time/0.002)),
                 "nstxout-compressed": str(int(run_time))}
        for i in range(len(delta)):
            cv_span["pull_coord" + str(i+1) +"_rate"] = delta[i] / run_time
        old_path = starter.steered(new_parameters = cv_span)
        new_path = starter.latest_path
        shutil.move(old_path + "topol.tpr", new_path + "topol.tpr")
        #shutil.move(old_path + "traj_comp.xtc", new_path + "traj_comp.xtc")
        if old_path != None:
    ...
    VIS_collection = []
    for index,i in enumerate(indexes): # FIXED? CALL TO CONSTRUCTOR
...
def plot_sim_par_coords(panda_frame, savedir = None, color = "Iter"):
    # Generates a parallel coordinate system and saves to html file
    fig = px.parallel_coordinates(panda_frame, color = color)
    if savedir == None:
        fig.show()
    else:

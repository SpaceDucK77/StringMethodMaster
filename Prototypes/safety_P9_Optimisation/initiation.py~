...
def p7_run(saves = None):
    ...
    if "iterations" not in saves:
        ...
        iterations = [saves["first_string"], saves["second_string"]]
...
def p8_run(saves = None):
    ...
    plot_iter_splines_2D(phie,
                         ...
                         saves["iterations"],
#                         select = 0,
                         select = list(range(len(saves["iterations"]))),
                         ppf = 2,
                         savedir = "plots")
    save(saves)'''
    frame = create_panda(sa,
                         ea,
                         istates,
                        saves["iterations"],
                         ("phi", "psi"),
                         "plots")
    plot_sim_par_coords(frame)
